...
trait While {
  ...
  def repeatState[A](evalB: A => Boolean)
    ...
    modify(doSomething).flatMap(_ =>
      inspect(evalB(_)).flatMap(if (_) {
        repeatState(evalB)(doSomething)
      } else {
        modify(identity)
      }))
  }
  ...
  def whileIO[A](initial: A)(evalB: A => IO[Boolean])
    ...
    /* cats iterateWhileM does not take evalB of such signature */
    evalB(initial).flatMap(if (_) {
      doSomething(initial).flatMap(whileIO(_)(evalB)(doSomething))
    } else {
      IO.pure(initial)
    })
  }
